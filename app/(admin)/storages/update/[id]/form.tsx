...
import { updateStorageSchema } from "@/types/storages";
...
import { useGetStorageById, useUpdateStorage } from "@/hooks/useStorages";
...
const types: TypeOf<typeof updateStorageSchema>["type"][] = [
  "RAW",
  "SEMIFINISHED",
  "FINISHED",
  "PRODUCTION",
...
export default function DataFrom({ id }: { id: number }) {
  ...
  const { data } = useGetStorageById(id);
  const { data: divisions } = useGetAllDivisions();

  const form = useForm<z.infer<typeof updateStorageSchema>>({
  ...
  function onSubmit(values: z.infer<typeof updateStorageSchema>) {
    mutate({ id, ...values });
  }
