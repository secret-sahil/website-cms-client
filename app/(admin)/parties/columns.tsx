...
import { PartiesResponse } from "@/types/parties";
...
const Cell = ({ data }: { data: PartiesResponse }) => {
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem asChild>
            <Link href={`/parties/update/${data.id}`} className="flex items-center">
...
export const columns: ColumnDef<PartiesResponse>[] = [
  {
    accessorKey: "id",
    ...
    cell: ({ row }) => (
      <span className="flex flex-wrap items-center gap-1">
        {row.original.divisions.map((division) => (
          <span key={division.division.id} className="bg-green-500/80 text-white px-1 rounded-md">
    ...
    cell: ({ row }) => (
      <span className="flex flex-wrap items-center gap-1">
        {row.original.type.map((type) => (
          <span key={type} className="bg-purple-500/80 text-white px-1 rounded-md">
            {type}
          </span>
        ))}
      </span>
